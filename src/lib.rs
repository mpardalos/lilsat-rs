...
#[derive(Debug, Clone)]
pub struct Valuation(pub Vec<Option<VarData>>);
...
impl Lilsat {
    ...
    pub fn solve(formula: &Formula) -> Answer {
...
/// Evaluates a clause under a valuation using three-valued logic
/// Returns Some(true) if ANY literal is true, Some(false) if ALL are false, None if undecided
pub fn eval_clause(valuation: &Valuation, clause: &Clause) -> Option<bool> {
    let mut has_none = false;

    for lit in &clause.0 {
        match valuation.eval_lit(*lit) {
            Some(true) => return Some(true), // Short-circuit on true
            Some(false) => continue,
            None => has_none = true,
        }
    }
...
/// Evaluates a formula under a valuation using three-valued logic
/// Returns Some(true) if ALL clauses are true, Some(false) if ANY clause is false, None if undecided
pub fn eval_formula(valuation: &Valuation, formula: &Formula) -> Option<bool> {
    let mut has_none = false;

    for clause in &formula.0 {
        match eval_clause(valuation, clause) {
            Some(false) => return Some(false), // Short-circuit on false
            Some(true) => continue,
            None => has_none = true,
        }
    }
    ...
    if has_none {
        ...
        Some(true) // All clauses are true
    }
}
